...
- F1 release-signing secrets may be consumed only by the existing `build-signed-rc` and `publish-github-release` jobs through the protected `release-signing` Environment; candidate validation, monitor jobs and temporary canaries must not access signing secrets or that Environment.
