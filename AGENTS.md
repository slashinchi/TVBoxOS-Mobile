...
- Autonomous GitHub Actions may execute only the already-authorized workflow contract; an agent's repository write permission does not authorize a workflow to broaden its own trust boundary. U2a's builder has no signing material, the new shared `rc-pipeline.yml` signer is its sole signer Environment consumer, and its verifier/attestor has no signing secrets.
...
- F1 release-signing secrets remain Environment-only. During U2a, the new shared RC path exposes them only to the isolated signer job in `rc-pipeline.yml`; candidate validation, ordinary CI, monitor jobs, verifier/attestor jobs and temporary canaries must not access them. The legacy tag publisher remains outside U2a and is removed in the later U2b authority transition.
- F1 canary refs/PRs are disposable shadow state; they must never approve/merge, touch real `main`/`patched`/tags/Releases, or leave a workflow/ref/PR behind. Only ChatGPT may close Automation Foundation; OC returns `REVIEW_PENDING` or `NEEDS_REPLAN`.
